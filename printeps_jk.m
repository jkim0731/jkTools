function printeps_jk(fignum, figname)
% a modified version from printeps. 
...
if strcmp(fignum, 'gcf')
    print(gcf, '-depsc', '-r600', sprintf('%s',figfilestr), '-painters')
elseif isnumeric(fignum)
    print(sprintf('-f%d',fignum), '-depsc', '-r600', sprintf('%s',figfilestr), '-painters')
else
    error('format error in using printeps_jk')
end
